        ...
        sidebarPanel(
            tabsetPanel(
                tabPanel("Data Options",
                         div(id="prev", 
                             h4("Previous Data"),
                             checkboxInput("load_prev", label = "Load data from previous session", value = FALSE),
...
server <- 
    ...
    shinyServer(function(input, output, session) {
        ...
        observeEvent(input$submit_analysis, {
            ...
            cchs_ready <- filter(cchs_df, DHH_AGE>=min(input$age_range), DHH_AGE<=max(input$age_range))
            
            cchs_ready$phu <- as.factor(ifelse(cchs_ready$GEODVHR4==input$phu,"Yes","No"))
            
            cchs_ready$peer <- as.factor(ifelse(cchs_ready$GEODVHR4!=input$phu & cchs_ready$GEODVPG==input$peer,"Yes","No"))
            
            cchs_ready$prov <- "Yes"
            
            cchs_ready$phu_vs_prov <- as.factor(ifelse(cchs_ready$GEODVHR4==input$phu,"phu","prov"))
            
            cchs_ready$phu_vs_peer <- as.factor(ifelse(cchs_ready$GEODVHR4==input$phu,"phu",ifelse(
                cchs_ready$GEODVPG==input$peer,"peer",NA)))
            ...
            svydesign_phu <- subset(svydesign, phu=="Yes")
            svydesign_peer <- subset(svydesign, peer=="Yes")
            
            results <<- 
                cchs_table(questions = in_questions,
                           svy_design_phu = svydesign_phu,
                           svy_design_peer = svydesign_peer,
                           svy_design_prov = svy_design,
                           by_vars = in_byvars,
                           crude = input$crude,
